clc; close all;
sim_data = sim('Project_180PWM_example.slx');
t = simout.time;
Tsigs = []
iqs = simout.data(:,1);
ids = simout.data(:,2);
...
for i = 0:5
    ...
    subplot(6,1,i+1); plot(t,simout.data(:,13+i),'-b'); xlabel("Time [s]"); xlim([.4 .6]); title(ptitle)
end  
saveas(gcf,"T1toT6_1_step.png")
...
saveas(gcf,"Project_iqs_ids_1_step.png")
figure
...
saveas(gcf,"Project_iasbscs_1_step.png")

figure
plot(t,Te); xlabel("Time [s]"); ylabel("Torque [Nm]"); title("Motor Torque")
saveas(gcf,"Project_Te1_step.png")
figure
plot(t,rpm); xlabel("Time [s]"); ylabel("RPM"); title("Motor Speed")
saveas(gcf,"Project_rpm1_step.png")

figure
plot(t,Vab); xlabel("Time [s]"); ylabel("Voltage"); xlim([.4 .6]); title("Voltage ab")
